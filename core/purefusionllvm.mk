# Copyright (C) 2017-2019 PureFusionLLVM
#
...
# PFLLVM module disable
DISABLE_PFLLVM_arm :=
DISABLE_PFLLVM_arm64 := libv8base libblasV8 libF77blasV8 libF77blas libbnnmlowpV8 libfdlibm libGLES_android libwebrtc% libmusicbundle libreverb \
                     camera.msm8996 libmmlib2d_interface libmmjpeg_interface libmmcamera_interface libqomx_core

# Set DISABLE_PFLLVM based on arch
DISABLE_PFLLVM := \
  $(DISABLE_PFLLVM_$(TARGET_ARCH)) \
  $(LOCAL_DISABLE_PFLLVM)

# Enable PureFusionLLVM on GCC modules.
#   Split up by arch.
ENABLE_PFLLVM_arm :=
ENABLE_PFLLVM_arm64 :=

# Set ENABLE_PureFusionLLVM based on arch
ENABLE_PFLLVM := \
  $(ENABLE_PFLLVM_$(TARGET_ARCH)) \
  $(LOCAL_ENABLE_PFLLVM)

# Enable PureFusionLLVM on current module if requested.
ifeq (1,$(words $(filter $(ENABLE_PFLLVM),$(LOCAL_MODULE))))
  my_cc := $(CLANG)
  my_cxx := $(CLANG_CXX)
  my_clang := true
endif

# Disable PureFusionLLVM on current module.
ifeq ($(my_clang),true)
  ifeq (1,$(words $(filter $(DISABLE_PFLLVM),$(LOCAL_MODULE))))
    my_cc := $(AOSP_CLANG)
