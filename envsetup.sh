...
function settitle()
{
    if [ "$STAY_OFF_MY_LAWN" = "" ]; then
        ...
        if [ -z "$apps" ]; then
            export PROMPT_COMMAND="echo -ne \"\033]0;[${arch}-${product}-${variant}] purefusionos@buildbox.com: ${PWD}\007\""
        else
            export PROMPT_COMMAND="echo -ne \"\033]0;[$arch $apps $variant] purefusionos@buildbox.com: ${PWD}\007\""
        fi
    fi
}
...
function addcompletions()
{
    ...
    dir="sdk/bash_completion vendor/pure/bash_completion"
    if [ -d ${dir} ]; then
